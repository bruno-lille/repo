...
def manual_add():
    ...
    tmdb_input = request.form.get("tmdb_input")
    ...
    cursor.execute("""
        INSERT INTO films (titre, emplacement, type, allocine, tmdb_id)
        VALUES (?, ?, ?, ?, ?)
    """, (title, emplacement, type_disc, allocine, tmdb_input))

    conn.commit()
    conn.close()

    # 🔥 retour à l'accueil avec recherche

...
def home():
    ...
    .retour {background:#555; color:white;}
    ...
    if results:
        ...
        for score, i in results:
            ...
            # 🔥 recherche bande annonce FR optimisée
            bande_annonce_url = "https://www.google.com/search?q=" + urllib.parse.quote(
                titres[i] + " bande annonce officielle VF allocine"
            )
            ...
            <div class="card">
                ...
                <div class="info">✔️ 📀 {col_C[i]} | ✔️ 📁 {col_A[i]} | ✔️ 📅 {annees[i]}</div>

                <div class="info">🎭 {genres[i]}</div>
                <div class="info">⭐ {casting[i]}</div>
                <div class="info">📝 {resumes[i]}</div>

                <div class="btn-row">
                    <a class="btn retour" href="javascript:history.back()">⬅️</a>
                    <a class="btn allocine" href="{liens[i]}" target="_blank">🎬</a>
                    <a class="btn allocine" href="{bande_annonce_url}" target="_blank">▶️</a>

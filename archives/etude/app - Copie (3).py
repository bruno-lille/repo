...
API_KEY = "YOUR_TMDB_API_KEY"  # facultatif pour fallback image
    ...
    try:
        ...
        if data["results"]:
            ...
            if film.get("poster_path"):
                return f"https://image.tmdb.org/t/p/w300{film['poster_path']}", film.get("id")
    except:
        pass

    return f"https://via.placeholder.com/300x450?text={urllib.parse.quote(query)}", ""
...
def home():
    ...
    .card {background:#1c1c1c; padding:15px; border-radius:16px; margin:15px auto; width:95%; max-width:420px; box-shadow: 0 4px 15px rgba(0,0,0,0.5);}
    img {width:100%; border-radius:12px; margin-bottom:10px;}
    .titre {font-size:20px; font-weight:bold; margin-bottom:8px;}
    .meta {color:#aaa; font-size:14px; margin-bottom:8px;}
    .info {color:#ddd; font-size:14px; margin:6px 0; text-align:left;}
    .resume {color:#bbb; font-size:13px; margin-top:8px; text-align:left;}
    ...
    if results:
        ...
        tmdb_search = f"https://www.themoviedb.org/search/movie?query={query_encoded}"

        bloc += f"""
        <div class="card">
            <div class="btn-row">
                <a class="btn allocine" href="{tmdb_search}" target="_blank">🔎 Autres résultats TMDB</a>
            </div>
        </div>
        """
    ...
    # ----------- AUCUN RESULTAT -----------
    img, tmdb_id = get_tmdb_image(query)

    tmdb_link = f"https://www.themoviedb.org/movie/{tmdb_id}" if tmdb_id else f"https://www.themoviedb.org/search/movie?query={urllib.parse.quote(query)}"
    allocine_search = f"https://www.google.com/search?q={urllib.parse.quote(query)}+allocine+fichefilm"
